...
# LastModified: 2018-09-26 11:33:00 +0900
#
...
class Post():
    ...
    def MakeInputData(self):
        ...
        # date
        Date = input("When?\nFormat:YYYY-MM-DD\n")
        if not re.match("^20[0-9]{2}-(0[1-9]|1[012])-(0[1-9]|[12][0-9]|3[01])",
                        Date):
        ...
        print("Account LIST")
        print(' '.join(
            list(self.Accounts[self.Accounts["active"] == 1]['name'])))

        if re.match("(payment|transfer)", self.Mode):
            Account = input("What is from Account?\n")
            try:
                self.Data['from_account_id'] = self.Accounts[
                    self.Accounts["name"] == Account]["id"].values[0]
            except IndexError:
        ...
        if re.match("(income|transfer)", self.Mode):
            ...
            try:
                self.Data['to_account_id'] = self.Accounts[
                    self.Accounts["name"] == Account]["id"].values[0]
            except IndexError:
                print("ERROR: Account don't exist")
                sys.exit(1)
        ...
        if re.match("(payment|income)", self.Mode):
            ...
            print(' '.join(
                list(self.Categories[self.Categories["mode"] == self.Mode]
                     ["name"])))
            Category = input("What is Category?\n")
            try:
                self.Data['category_id'] = self.Categories[
                    self.Categories["name"] == Category]["id"].values[0]
            except IndexError:
            ...
            if self.Mode == 'payment':
                ...
                print(' '.join(
                    list(self.Genres[self.Genres["category_id"] == self.
